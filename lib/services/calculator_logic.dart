...
class CalculatorLogic {
  ...
  static CalculatorState processInput(CalculatorState state, String input) {
    switch (input) {
      ...
      case "x²":
      case "1/x":
        return _handleScientific(state, input);
      case "π":
  ...
  static CalculatorState _handleClear(CalculatorState state) {
    return const CalculatorState(display: "0", memory: 0);
  }
  ...
  static CalculatorState _handleScientific(
    CalculatorState state,
    String function,
  ) {
    final input = state.currentValue ?? 0.0;
    double result;

    try {
      switch (function) {
        case "sin":
          result = math.sin(_toRadians(input));
          break;
        case "cos":
          result = math.cos(_toRadians(input));
          break;
        case "tan":
          result = math.tan(_toRadians(input));
          break;
        case "log":
          if (input <= 0) return state.copyWith(display: errorMessage);
